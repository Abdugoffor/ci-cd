...
@section('content')
    ...
    <div class="content">
        <div class="d-inline-flex gap-2">
            <a href="{{ route('aferta.index', [], false) }}" class="btn btn-sm btn-outline-secondary">
                {{ getTranslation('back') }}
            </a>
        </div>
        <div class="card mt-2">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('aferta.update', $aferta->id, false) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <fieldset class="mb-3">
                                    ...
                                    @foreach (getLanguage() as $model)
                                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                            ...
                                            <textarea class="form-control summernote" name="text[{{ $model->slug }}]" data-dashlane-classification="other"
                                                placeholder="{{ $model->name }}">{{ old('text.' . $model->slug, $aferta->text[$model->slug] ?? '') }}</textarea>
