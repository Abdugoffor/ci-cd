    ...
    <div class="content">
        ...
        <div class="card mt-2">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('faqs.update', $faq->id, false) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <fieldset class="mb-3">
                                    ...
                                    @foreach (getLanguage() as $model)
                                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                            ...
                                            <input type="text" class="form-control" name="question[{{ $model->slug }}]"
                                                value="{{ old('question.' . $model->slug, $faq->question[$model->slug] ?? '') }}"
                                            ...
                                            <!-- answer maydoni -->
                                            <label class="col-form-label col-lg-2">{{ getTranslation('answer') }}</label>
                                            <textarea class="form-control" name="answer[{{ $model->slug }}]" data-dashlane-classification="other"
                                                placeholder="{{ $model->name }}">{{ old('answer.' . $model->slug, $faq->answer[$model->slug] ?? '') }}</textarea>
