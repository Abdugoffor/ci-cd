...
@section('content')
    ...
    <div class="content">
        <div class="d-inline-flex gap-2">
            <a href="{{ route('categories.index', [], false) }}" class="btn btn-outline-secondary">
                {{ getTranslation('back') }}
            </a>
        </div>
        <div class="card mt-2">
                                ...
                                <div class="tab-content">
                                    @foreach (getLanguage() as $model)
                                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                            ...
                                            <textarea class="form-control" name="description[{{ $model->slug }}]" placeholder="{{ $model->name }}">{{ old('description.' . $model->slug) }}</textarea>
