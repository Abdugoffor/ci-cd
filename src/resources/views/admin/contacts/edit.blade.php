                ...
                <form action="{{ route('contacts.update', $contact->id,  false) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <fieldset class="mb-3">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="text" class="form-control" name="path" value="{{ $contact->path }}"
                                    ...
                                    @foreach (getLanguage() as $model)
                                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                            ...
                                            <input type="text" class="form-control" name="title[{{ $model->slug }}]"
                                                value="{{ $contact->title[$model->slug] ?? $contact->title['default'] }}"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="file" class="form-control" name="photo" value="{{ $contact->photo }}"
                                    placeholder="{{ getTranslation('photo') }}">
                                @error('photo')
                                    <p style="color: red;">{{ $message }}</p>
                                @enderror
                                <img src="{{ asset($contact->photo) }}" width="100px" class="mt-1" alt="">
